...
const Footer2 = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 bg-[#184F75]">
      <div className="xs:gap-8 grid grid-cols-4 gap-4 gap-y-8 py-8 md:py-12">
          ...
          >
            ...
            {texts &&
              texts.map((text, index2) => (
                <p key={`item-text-${index2}`} className="text-white dark:text-white">
                  {text}
                </p>
              ))}
          </div>
        ...
        <div className="col-span-4 sm:col-span-2 md:col-span-2 lg:col-span-1 xl:col-span-1">
          <div className="mb-2 font-medium text-white dark:text-white">Social</div>
          <ul className="mb-4 -ml-2 rtl:ml-0 rtl:-mr-2 flex md:order-1 md:mb-0">
            {socials.map(({ label, icon: Icon, href }, index) => (
              <li key={`item-social-${index}`}>
                <a
                  className="text-white hover:text-gray-700 inline-flex items-center rounded-lg p-2.5 text-sm hover:bg-gray-100 focus:outline-none focus:ring-4 focus:ring-gray-200"
                  aria-label={label}
                  href={href}
                >
      ...
      <div className="text-muted py-6 text-sm text-white dark:text-white md:flex md:items-center md:justify-between md:py-8">
